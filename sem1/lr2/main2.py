...
def main(targets: list, value: int) -> tuple[int, int]:  # функция реализации бинарного поиска
    ...
    Проверка на корректность типов данных осуществляется в функции user_input()
    ...
    Возвращения:
        tuple[int, int] или None:
            - None, если число не найдено в списке или нижняя граница больше верхней
    ...
    if low <= high and value in targets:
...
def user_input() -> None:  # функция взаимодействия с пользователем
    ...
    """
    while True: # для прерывания цикла при ошибке
