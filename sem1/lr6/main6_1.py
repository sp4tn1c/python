...
def benchmark(func, data, number=1, repeat=5):
    ...
    Для каждого значения в data функция выполняется несколько раз (repeat)
    с указанным количеством повторов (number), затем берется минимальное время.

    Аргументы:
        func (callable): Функция для тестирования
        data (list): Список входных данных (высот деревьев)
        number (int): Количество выполнений функции за один замер
        repeat (int): Количество повторных замеров

    Возвращения:
        float: Среднее минимальное время выполнения в секундах
    """

    total = 0
    for n in data:

        times = timeit.repeat(lambda: func(n, 8), number=number, repeat=repeat)
        total += min(times)
...
def main():
    ...
    for n in test_data:
        ...
        res_recursive.append(benchmark(gen_bin_tree_recursive, [n], number=10, repeat=3))
        res_iterative.append(benchmark(gen_bin_tree_iterative, [n], number=10, repeat=3))
